    ...
    if check_url(args.url):
        ...
        if check_drm(data):
            ...
            if args.list_formats:
                ...
                pre_download_serie(
                    data, args.seasons, args.episodes, args.format, args.out_dir
                )
            else:
...
def get_override_url(data, format):  # Generate the mp4 video url
    ...
    if format == "best":
        for format in formats:
            ...
            try:
                if (
                    requests.get(url_override, stream=True).headers["Content-Type"]
                    == "video/mp4"
                ):
                    return url_override
            except:
                sys.exit(
                    "[error] Connection error or the title has Verimatrix DRM protection."
                )
...
def get_definition(format):  # Retrive the video quality
    if debug:
        print("[debug] Getting DEFINITION")

    for bit in range(len(formats)):
        if formats[bit] == format:
            if debug:
                print("[debug] " + resolutions[bit])
...
def path_and_down(
    url, out_dir, file_name
):  # Check output file path and start the download
    out_dir = (
        out_dir[2:]
        .replace(":", " -")
        .replace("<", " ")
...
def pre_download(data, format, out_dir):  # Get all the infos to start the download
    if debug:
        print("[debug] Starting PRE-DOWNLOAD")

    if "Page" in data["id"]:
        data = get_json(url_root + data["first_item_path"])

    if debug:
        print("[debug] Defining METADATA")
...
def pre_download_serie(
    data, def_seasons, def_episodes, format, out_dir
):  # Get all the infos to start the download
    if debug:
        print("[debug] Starting PRE-DOWNLOAD SERIE\n")
    ...
    if "ContentItem" in data["id"]:
        ...
        for block in range(len(data["blocks"])):
            ...
            if (
                "Episodi" == blocks_name
                or "Puntate" == blocks_name
                or "Lingua italiana" == blocks_name
            ):
                seasons = []

                if debug:
                    print("[debug] Sorting SEASONS")
                ...
                for season in range(len(data["blocks"][block]["sets"])):
                    seasons.append(
                        data["blocks"][block]["sets"][season]["name"]
                        + "_SEP_"
                ...
                if debug:
                    print("[debug] Getting custom SEASONS")
                ...
                for sor_season in seasons:
                    ...
                    if debug:
                        print("[debug] Defining seasons METADATA\n")
                    fn_season = season_data["items"][0]["season"]
                    sub_dir = "%s (%s)\\Season %s" % (
                        fn_serie.strip(),
                        fn_year,
                        fn_season,
                    )
                    ...
                    if def_episodes[0] != "all":
                        ...
                        for def_episode in def_episodes:
                            for episode in range(len(episodes)):
                                if def_episode == episodes[episode]:
                                    if debug:
                                        print("[debug] Defining episode METADATA")
                                    fn_episode = season_data["items"][episode][
                                        "episode"
                                    ]
                                    fn_episode_title = season_data["items"][episode][
                                        "episode_title"
                                    ]
                                    url = get_override_url(
                                        get_json(
                                            url_root
                                            + season_data["items"][episode]["weblink"]
                                        ),
                                        format,
                                    )
                        ...
                        for episode in range(len(episodes)):
                            ...
                            url = get_override_url(
                                get_json(
                                    url_root + season_data["items"][episode]["weblink"]
                                ),
                                format,
                            )
...
def list_formats_serie(data, def_seasons, def_episodes):  # List the formats
    if debug:
        print("[debug] Listing FORMATS SERIE\n")
    try:
        if "ContentItem" in data["id"]:
            serie = data["program_info"]["name"]
            season = data["season"]
            ...
            print(
                'Formats avaiable for "%s - %sx%s - %s (%s)"'
                % (
                    serie.strip(),
                    season.zfill(2),
                    episode.zfill(2),
            ...
            if debug:
                print("[debug] Getting SEASONS")
            for block in range(len(data["blocks"])):
                if (
                    "Episodi" == data["blocks"][block]["name"]
                    or "Puntate" == data["blocks"][block]["name"]
                ):
                    seasons = []
                    if debug:
                        print("[debug] Sorting SEASONS")
                    ...
                    for sor_season in seasons:
                        ...
                        for episode in range(len(season_data["items"])):
                            if not season_data["items"][episode]["episode"] == "":
                                episodes.append(
                                    season_data["items"][episode]["episode"]
                                )
                            else:
                                episodes.append(episode + 1)
                            ...
                            for def_episode in def_episodes:
                                for episode in range(len(episodes)):
                                    if def_episode == episodes[episode]:
                                        ...
                                        fn_episode_title = season_data["items"][
                                            episode
                                        ]["episode_title"]

                                        print(
                                            'Ep %s - "%s"'
                                            % (fn_episode, fn_episode_title.strip())
                                        )
                                        url = season_data["items"][episode]["video_url"]
                                        for format in range(len(formats)):
                                            override_url = (
                                                url + override + formats[format]
                                            )
                                            r = requests.get(override_url, stream=True)
                                            if r.headers["Content-Type"] == "video/mp4":
                                                print(
